...
class MobileContextMenuContainer extends React.Component {
  ...
  handleClickOutside = (event) => {
    if (
      this.props.mobileContextMenu.open &&
      this.wrapperRef &&
  ...
  renameOnClick = async () => {
    let inputValue = this.props.mobileContextMenu.isFile
      ? this.props.mobileContextMenu.data.filename
      : this.props.mobileContextMenu.data.name;
    ...
    this.props.mobileContextMenu.isFile
      ? this.props.dispatch(
          startRenameFile(
            this.props.mobileContextMenu.data._id,
            folderName,
            this.props.mobileContextMenu.data.metadata.drive
          )
        )
      : this.props.dispatch(
          startRenameFolder(
            this.props.mobileContextMenu.data._id,
            folderName,
            this.props.mobileContextMenu.data.drive,
            this.props.mobileContextMenu.data.parent
          )
        );
    this.props.dispatch(resetMobileContextMenu());
  };
  ...
  shareFileOnClick = () => {
    this.props.dispatch(
      setShareSelected({ ...this.props.mobileContextMenu.data })
    );
    this.props.dispatch(resetMobileContextMenu());
  };
  ...
  moveOnClick = () => {
    ...
    const parent = this.props.mobileContextMenu.isFile
      ? this.props.mobileContextMenu.data.metadata.parent
      : this.props.mobileContextMenu.data.parent;
    const isFile = this.props.mobileContextMenu.isFile;
    const drive = this.props.mobileContextMenu.isFile
      ? this.props.mobileContextMenu.data.metadata.drive
      : this.props.mobileContextMenu.data.drive;
    const personalItem = this.props.mobileContextMenu.isFile
      ? this.props.mobileContextMenu.data.metadata.personalFile
      : this.props.mobileContextMenu.data.personalFolder;
  ...
  deleteOnClick = () => {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.props.mobileContextMenu.isFile
          ? this.props.dispatch(startRemoveFile(id))
          : this.props.dispatch(
              startRemoveFolder(
                id,
                [...this.props.mobileContextMenu.data.parentList, id],
                drive,
                parent,
                personalItem
              )
  ...
  render() {
    return (
      <div
        className="mobile__context-menu__wrapper"
        ref={this.wrapperRef}
        style={this.props.mobileContextMenu.open ? {} : { display: "none" }}
      >
        <div
          onClick={this.renameOnClick}
          className="mobile__context-menu__item"
        >
          <img
            className="mobile__context-menu__image"
            src="/assets/filesetting1.svg"
            alt="setting"
          />{" "}
          <p className="mobile__context-menu__title">Rename</p>
        </div>
        {this.props.mobileContextMenu.isFile ? (
          <div
            onClick={this.shareFileOnClick}
            className="mobile__context-menu__item"
          >
            <img
              className="mobile__context-menu__image"
              src="/assets/filesetting2.svg"
              alt="setting"
            />{" "}
            <p className="mobile__context-menu__title">Share</p>
          </div>
        ) : undefined}
        {this.props.mobileContextMenu.isFile ? (
          <div
            onClick={this.downloadFileOnClick}
            className="mobile__context-menu__item"
          >
            <img
              className="mobile__context-menu__image"
              src="/assets/filesetting3.svg"
              alt="setting"
            />{" "}
            <p className="mobile__context-menu__title">Download</p>
          </div>
        ) : undefined}
        <div onClick={this.moveOnClick} className="mobile__context-menu__item">
          <img
            className="mobile__context-menu__image"
            src="/assets/filesetting4.svg"
            alt="setting"
          />{" "}
          <p className="mobile__context-menu__title">Move</p>
        </div>
        <div
        ...
        >
          <img
            className="mobile__context-menu__image"
            src="/assets/filesetting5.svg"
            alt="setting"
          />{" "}
          <p className="mobile__context-menu__title">Delete</p>
        </div>
      </div>
    );
  }
...
export default connect(connectPropToStore)(withRouter(MobileContextMenuContainer));
