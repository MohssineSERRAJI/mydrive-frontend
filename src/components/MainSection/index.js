import { startLoadMoreFiles, startSetFileAndFolderItems } from "../../actions/files";
import {
...
class MainSectionContainer extends React.Component {
  ...
  folderClick = (id, bypass = false) => {
    ...
    if (
      ...
      this.props.dispatch(startSetSelectedItem(id, false, false));
    }
  };
  ...
  downloadFile = (fileID, file) => {
    ...
    axios
      .get(`/file-service/download/${fileID}`)
      .then((response) => {
...
export default connect(connectPropToState)(withRouter(MainSectionContainer));
