...
class RightSectionContainer extends React.Component {
  ...
  getTranscodeButton = (props) => {
    if (!props.selectedItem.is_video || !env.enableVideoTranscoding) {
      return undefined;
    }
  ...
  handleClickOutside = (e) => {
    if (
      ...
      !this.rightSectionRef.current.contains(e.target)
    ) {
  ...
  startMoveFolder = async () => {
    const parent = this.props.selectedItem.data.parent
    const isPersonal = this.props.selectedItem.file
      ? this.props.selectedItem.data.personalFile
  ...
  changeDeleteMode = async () => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.props.selectedItem.file
          ? this.props.dispatch(startRemoveFile(this.props.selectedItem.id))
          : this.props.dispatch(
              startRemoveFolder(
                this.props.selectedItem.id,
                [
