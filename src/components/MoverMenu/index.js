...
class MoverMenuContainer extends React.Component {
  ...
  search = (e) => {
    ...
    const url = `/folder-service/list?parent=${id}&search=${searchValue}&itemType=nonpersonal&sortBy=DEFAULT`;

    axios.get(url).then((response) => {
      let folders = response.data;

      if (!this.props.isFile)
        folders = folders.filter((folder) => folder.pk !== this.props.ID);
  ...
  goHome = () => {
    ...
    const url = `/folder-service/list?parent=${id}&itemType=nonpersonal`;
    ...
    axios
      ...
      .then((response) => {
        ...
        if (!this.props.isFile)
          folders = folders.filter((folder) => folder._id !== this.props.ID);
  ...
  goBack = () => {
    ...
    if (this.state.historyList.length > 1) {
      ...
      currentName =
        this.state.historyList[this.state.historyList.length - 1].name;
    } else if (this.state.historyList.length !== 0) {
      historyList = this.state.historyList;
      historyList.pop();
    }
    ...
    const url = this.props.isGoogle
      ? `/folder-service-google/list?parent=${currentID}`
      : this.props.isPersonal
      ? `/folder-service/list?parent=${currentID}&itemType=personal`
      : `/folder-service/list?parent=${currentID}&itemType=nonpersonal`;
  ...
  folderClick = (id, name) => {
    ...
    if (lastTimeDifference < 1500 && id === this.state.selected) {
      const url = this.props.isGoogle
        ? `/folder-service-google/list?parent=${id}`
        : this.props.isPersonal
        ? `/folder-service/list?parent=${id}&itemType=personal`
        : `/folder-service/list?parent=${id}&itemType=nonpersonal`;
