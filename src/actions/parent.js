...
export const startSetParentList = (id, isGoogle=false) => {
    ...
    return (dispatch) => {
        ...
        if (cachedResults[id]) {
            console.log("cahce0", cachedResults[id])
        ...
        const URL =`/folder-service/info/${id}`;

        dispatch(setParentList(["/", ""], ["Home", ""]))

        axios.get(URL).then((response) => {

            const parentList = ["/", response.data[0].pk];
            const parentNameList = ["Home", response.data[0].fields.name];
            dispatch(setParentList(parentList, parentNameList));

            cachedResults[id] = {_id: response.data[0].pk, name: response.data[0].fields.name};
